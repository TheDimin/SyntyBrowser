...
- Uses developer-local preview PNGs generated by an asynchronous, headless Blender process; browsing never imports, compiles, or renders a model in s&box just to paint a thumbnail.
...
Visible cards queue `Tools/generate-preview.ps1` in a background process and show a cheap placeholder until its PNG appears. The script can also be run directly outside s&box and requires Blender 4.5. For MaterialList assets, pass `-BindingsJson` containing `MeshName`, `SlotOrdinal`, and `TextureHint` values. The producer matches the MaterialList mesh name and slot ordinal to the imported Blender mesh slot, preserving the FBX UV layers and independent multi-material assignments. It never binds authoritative textures by FBX material object name.
