...
- Uses developer-local, externally generated preview PNGs; browsing never imports or compiles a model just to paint a thumbnail.
...
Run `Tools/generate-preview.ps1` outside s&box for individual assets. The script requires [F3D](https://f3d.app/). Pass texture hints in MaterialList slot order so preview material selection remains authoritative; fallback FBXs may use their embedded material names.
